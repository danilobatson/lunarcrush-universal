// 🛠️ LunarCrush Service Fixes (Imported from Working Yoga Backend)
...
export async function getNftTimeSeriesV1Fixed(
	config: LunarCrushConfig,
	args: { id: string; interval?: string; start?: number; end?: number }
): Promise<any[]> {
	const { id, interval, start, end } = args;
	try {
		...
		const response = await fetch(
			`${config.baseUrl || 'https://lunarcrush.com/api4/public'}/nfts/${id}/time-series?${params}`,
			{
				headers: {
...
export async function getSearchFixed(
	config: LunarCrushConfig,
	id: string
): Promise<any> {
	try {
		const response = await fetch(
			`${config.baseUrl || 'https://lunarcrush.com/api4/public'}/search/${id}`,
			{
				headers: {
					Authorization: `Bearer ${config.apiKey}`,
					'Content-Type': 'application/json',
...
export async function searchPostsFixed(
	config: LunarCrushConfig,
	term: string
): Promise<any[]> {
	try {
		...
		const response = await fetch(
			`${config.baseUrl || 'https://lunarcrush.com/api4/public'}/posts/search?${params}`,
			{
				headers: {
					Authorization: `Bearer ${config.apiKey}`,
					'Content-Type': 'application/json',
...
export async function getPostDetailsFixed(
	config: LunarCrushConfig,
	id: string
): Promise<any> {
	try {
		const response = await fetch(
			`${config.baseUrl || 'https://lunarcrush.com/api4/public'}/posts/${id}`,
			{
				headers: {
					Authorization: `Bearer ${config.apiKey}`,
					'Content-Type': 'application/json',
...
export async function getPostTimeSeriesFixed(
	config: LunarCrushConfig,
	args: {
		id: string;
		bucket?: string;
  ...
  const { id, bucket, interval, start, end } = args;
	try {
		const params = new URLSearchParams({
			id,
			...(bucket && { bucket }),
		...
		const response = await fetch(
			`${config.baseUrl || 'https://lunarcrush.com/api4/public'}/posts/${id}/time-series?${params}`,
			{
				headers: {
					Authorization: `Bearer ${config.apiKey}`,
					'Content-Type': 'application/json',
