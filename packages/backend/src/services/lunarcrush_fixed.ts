...
export const getCrypto = async (
	...
	try {
		console.log(
			`🔍 getCrypto: ${symbol} (FIXED: using list/v2 endpoint for complete social data)`
		);
		...
		if (response.data && response.data.length > 0) {
			console.log(
				`✅ getCrypto SUCCESS: Found ${response.data.length} result(s) for ${symbol}`
			);
			console.log(
				`🎯 Social metrics: sentiment=${response.data[0].sentiment}, social_dominance=${response.data[0].social_dominance}, interactions_24h=${response.data[0].interactions_24h}`
			);
